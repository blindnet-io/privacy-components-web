// derivative of https://github.com/CleverCloud/clever-components/blob/9bf1b69/src/stories/lib/markdown.cjs
// by CleverCloud
...
const {
  storybookPlugin: modernWebStorybookPlugin,
} = require('@web/dev-server-storybook');
...
function markdownToCsfWithDocsPage(markdownText) {
  ...
  const frontmatterNode = markdownAst.children.find(
    node => node.type === 'yaml'
  );
  ...
  const title =
    [kind, subtitle].filter(a => a != null).join('/') ||
    markdownAst.children
      .find(node => node.type === 'heading')
      .children.find(node => node.type === 'text').value;
