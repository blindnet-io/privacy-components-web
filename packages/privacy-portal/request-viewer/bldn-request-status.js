...
const cancelSvg = new URL(new URL('../assets/ic_round-cancel.svg', import.meta.url).href, import.meta.url).href;
const refreshSvg = new URL(new URL('../assets/heroicons-solid_refresh.svg', import.meta.url).href, import.meta.url).href;
/**
...
let BldnRequestStatus = class BldnRequestStatus extends CoreConfigurationMixin(LitElement) {
    constructor() {
        super(...arguments);
    ...
    handleDownloadClick(demand) {
        window.location.href = demand.data;
    }
    handleCancelClick(demand) {
        ComputationAPI.getInstance()
    ...
    render() {
      ...
      <bldn-dropdown class="main-section" mode="major" open>
        <span slot="heading" id='summary-heading'><b>${msg(html `Request Summary`)}</b>
          <bldn-button mode='link-icon' underline-mode='none' @bldn-button:click=${this.handleCopyLinkClick}>
        ...
        ${when(this._requestDetails.length > 0, () => html `
            ...
            return html `
                <bldn-dropdown>
                  <span slot="heading"
                    ><strong
                      >${msg(html `${ACTION_TITLES_WITH_DEMAND[demand.requested_action]()}`)}&nbsp;&nbsp;&nbsp;<span class=${classMap(classes)}
                ...
                const subClasses = {
                    granted: subDemand.status ===
                        PrivacyResponsePayload.status.GRANTED,
                    denied: subDemand.status === PrivacyResponsePayload.status.DENIED,
                    partial: subDemand.status ===
                        PrivacyResponsePayload.status.PARTIALLY_GRANTED ||
                        subDemand.status ===
                            PrivacyResponsePayload.status.UNDER_REVIEW,
                    canceled: subDemand.status ===
    ...
    getStatusTemplate(demand) {
        return html `
      <p>
        <b>${msg('Requested:')}</b> ${ACTION_DESCRIPTIONS[demand.requested_action]()}
      </p>
      ...
      ${choose(demand.requested_action, [
          ...
          `,
            ...
            [
                ...
                () => html `
            <p>
              ${msg('Your transparency demand has been granted and split into multiple demands below.')}
            </p>
          `,
            ],
            [
...
], BldnRequestStatus.prototype, "requestId", void 0);
__decorate([
    state()
], BldnRequestStatus.prototype, "_requestDetails", void 0);
__decorate([
    state()
], BldnRequestStatus.prototype, "_error", void 0);
BldnRequestStatus = __decorate([
    customElement('bldn-request-status')
], BldnRequestStatus);

export { BldnRequestStatus, getRequestLink, getRetentionPolicyString };
//# sourceMappingURL=bldn-request-status.js.map
