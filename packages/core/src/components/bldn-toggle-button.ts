...
export class ToggleButton extends LitElement {
  @property({ type: String, attribute: 'left-text' }) leftText: string = '';

  @property({ type: String, attribute: 'right-text' }) rightText: string = '';

  @property({ type: String, attribute: 'left-id' }) leftId: string | undefined =
    undefined;

  @property({ type: String, attribute: 'right-id' }) rightId:
    | string
    | undefined = undefined;
  ...
  handleClick(side: 'left' | 'right') {
    this.dispatchEvent(new Event(`toggle-button-${side}-click`));
    this.selected = side;
  }
